...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mAuthListener = new FirebaseAuth.AuthStateListener() {
            @Override
            public void onAuthStateChanged(FirebaseAuth firebaseAuth) { //Comprueba tanto cuando iniciamos sesion como cuando la cerramos
                final FirebaseUser user = firebaseAuth.getCurrentUser(); //FirebaseAuth.getInstance().getCurrentUser();

                if (user != null) {
                    myRef.child(user.getUid()).child("verificado").addValueEventListener(new ValueEventListener() {
                        @Override
                        public void onDataChange(DataSnapshot dataSnapshot) {
                            if (dataSnapshot.exists()) {
                                boolean verificado = (boolean) dataSnapshot.getValue();
                                if (verificado == true) {
            ...
            public void onComplete(Task<AuthResult> task) {
                if (task.isSuccessful()) {
                    ...
                    if (user.isEmailVerified()) {
                        ...
                        myRef.child(key).child("verificado").setValue(true);
                        Intent i = new Intent(MainActivity.this, MapaActivity.class);
                        startActivity(i);
                    } else {
                    ...
                    errorSignIn.setMessage(R.string.usuario_contrasena_incorrecto)
                            .setCancelable(false)
                            .setPositiveButton(R.string.aceptar, new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialog, int which) {
