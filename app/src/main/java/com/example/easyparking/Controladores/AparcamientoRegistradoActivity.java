        ...
        if (requestCode == REQUEST_CODE_PAYMENT) {
            if (resultCode == RESULT_OK) {
                ...
                if (confirmation != null) {
                    ...
                    //Si el estacionamiento es en un parking borrarlo de la base de datos
                    if (stringZona.equalsIgnoreCase("Parking")) { //Si el coche esta aparcado en la zona gratis

                        String key = stringMatricula.concat("_").concat(stringFecha).concat(" ").concat(stringHora);
                        myRef.child(user.getUid()).child("aparcamientos en curso").child(key).removeValue();
                        dialog.cancel();
                        finish();
                    } else {
                        //Poner en aparcamiento como finalizado en la base de datos
